    ...
    def set_openemail_mode():
        ...
        if lang_exists == True:
            ...
            if msg == '1':
                ...
                while not entered_mailopencode_right:
                    open_email_mode = input('Please choose your mode of loading an email\n1.txt\n2.html)')
                    if open_email_mode == '1':
                        f2 = open('emailloadmethod',encoding='utf-8',mode='w')
                        f2.write(open_email_mode)
                        f2.close()
        ...
        while not entered_mailopencode_right:
            ...
            if open_email_mode == '1':
                ...
                f2.write(open_email_mode)
                f2.close()
                print('您的加载邮件方式已被设为纯文本。Your method of loading emails have been set to txt.')
                entered_mailopencode_right = True
                time.sleep(2)
            elif open_email_mode == '2':
                f2 = open('emailloadmethod',encoding='utf-8',mode='w')
                f2.write(open_email_mode)
                f2.close()
                print('您的加载邮件方式已被设为HTML。Your method of loading emails have been set to html.')
                entered_mailopencode_right = True
                time.sleep(2)
            else:
